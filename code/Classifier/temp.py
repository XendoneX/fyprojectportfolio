...
cputarget=dataset['cpuTargets']
hddtarget=dataset['hddTargets']
...
class classifier:
    ...
    def Predict(mu, sigma, pdf, data, pA, pM): #Predicts the whether the provided data/variable is malicious or not
        ...
        for xi in data:
            ...
            if math.isnan(float(p) or pA==0.0): #Checks if values are NaN, if this is true, check values without PDF 
...
#Create Model
    model=classifier
    loadPredict = model.CpuLoad(cpuload)
    tempPredict = model.CpuTemp(cputemp)
    powerPredict = model.CpuPower(cpupower)
    hddPredict = model.HddTemp(hddtemp)
    
    print(model.Accuracy(cputarget, loadPredict))
    print(model.Accuracy(cputarget, tempPredict))
    print(model.Accuracy(cputarget, powerPredict))
    print(model.Accuracy(hddtarget, powerPredict))